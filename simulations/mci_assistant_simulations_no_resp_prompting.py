...
def log_conversation_to_file(chat_history, filename_index): 
    ...
    filename = f"conversation_samples_no_resp_prompting/mci_conversation_{filename_index}.txt" 
    
    # Ensure the directory exists before writing the file
    os.makedirs("conversation_samples_no_resp_prompting", exist_ok=True)
...
for filename in os.listdir("image_learning_results"):
    ...
    while random.random() < 0.7: #random.random() (uniform in [0,1)) with <0.7 will give a bernoulli trial with p=0.7
        chat_history_assistant.append({"role": "user", "content": ""})
        response_assistant = client.chat.completions.create(
            model="gpt-4o",  # or use the relevant model
            messages=chat_history_assistant
        )
